...
async function chargerCatalogue() {
    try {
        const films = await fetchFromTMDB(currentEndpoint, currentPage, currentQuery);
    ...
    films.forEach(movie => {
        ...
        const cardHTML = `
            <div class="card bg-base-100 shadow-xl hover:scale-105 transition-transform duration-200">
                <figure>
                    <img src="${imagePath}" alt="${movie.title}" class="w-full h-auto object-cover" />
                </figure>
                <div class="card-body p-4">
                    <h2 class="card-title text-base">
                        ${movie.title}
                        <div class="badge badge-secondary text-xs">${movie.vote_average.toFixed(1)}</div>
                    </h2>
                    <p class="text-xs text-gray-500 line-clamp-3">${movie.overview}</p>
                    <div class="card-actions justify-end mt-2">
                        <button onclick="openDetail('movie', ${movie.id})" class="btn btn-primary btn-sm">Détails</button>
                    </div>
                </div>
            </div>
        `;
